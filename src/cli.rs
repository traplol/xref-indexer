...
use crate::db;
use crate::types::{CallEdge, Definition, Location, SymbolKind, Visibility};
use crate::{Indexer, Language, ReferenceMode};
use rusqlite::types::Value as SqlValue;
use rusqlite::{params, params_from_iter, Connection, OpenFlags, OptionalExtension, Row};
use serde::Serialize;
use serde_json::{json, Value};

pub const DEFAULT_DB_PATH: &str = ".git/code-indexer/xrefs.sqlite3";
...
pub fn run_from_env() {
    ...
    let result = run_cli_args_with_pretty(&args);
    match result {
        Ok((value, pretty)) => emit_json(value, pretty),
        Err(err) => exit_json_error(&err),
    }
}

pub fn run_cli_args(args: &[String]) -> Result<Value, String> {
    run_cli_args_with_pretty(args).map(|(value, _)| value)
}

fn run_cli_args_with_pretty(args: &[String]) -> Result<(Value, bool), String> {
    if args.is_empty() {
    ...
    let pretty = parsed.bool_flag("pretty");
    dispatch_command(command, &parsed).map(|value| (value, pretty))
}

fn dispatch_command(command: &str, parsed: &ParsedArgs) -> Result<Value, String> {
    match command {
